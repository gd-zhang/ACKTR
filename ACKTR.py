...
class ACKTR:
    def __init__(self, sess, args):
        self.args = args
        # TODO: check all hyper-parameters
        optimizer_params = {'learning_rate': args.learning_rate, 'moving_average': args.moving_average,
                            'damping': args.damping, 'kl_clip': args.kl_clip, 'momentum': 0.9}
        self.model = Model(sess, optimizer_params=optimizer_params, args=self.args)
        self.trainer = Trainer(sess, self.model, args=self.args)
        self.env_class = ACKTR.env_name_parser(self.args.env_class)

    def train(self):
        env = ACKTR.make_all_environments(self.args.num_envs, self.env_class, self.args.env_name,
                                          self.args.env_seed)
    ...
    def make_all_environments(num_envs=4, env_class=None, env_name="SpaceInvaders", seed=42):
        ...
        return SubprocVecEnv(
            [ACKTR.__env_maker(env_class, env_name, i, seed) for i in range(num_envs)])
